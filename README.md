...
4. **세법 개정 감시** (`src/monitor/law-watch.js`) — 엔진이 박아둔 세법 가정(공정시장가액비율·공제금액·중과 부활일 등 상수 8종 + 판결·심판/정부안·규제지역 동향 2종)을 웹검색으로 대조해 **바뀐 항목과 고쳐야 할 상수 위치**를 경고합니다. 케이스와 무관하므로 `scripts/law-watch.js`로 **단독 실행**하며, `law-watch` GitHub Actions 워크플로가 **매주 월·수·금 06:00 KST에 자동 실행**되어 개정이 발견되면 잡이 실패(빨간불)하고 `law-watch` 라벨 이슈가 자동 생성됩니다.
...
정기 감시는 `law-watch` 워크플로가 매주 월·수·금 06:00 KST에 자동 실행합니다. 개정·시행예정 항목이 발견되면 잡이 실패해 GitHub 알림이 가고, `law-watch` 라벨 이슈가 자동 생성되며, 리포트에 고쳐야 할 상수 위치(`constants.*`)가 표시됩니다. 발견 내용은 검토 후 세법 동향 일지(`src/web/static/law-updates-data.js`)에 일자별로 기록합니다.
...
| **세법 동향** | `law-updates.html` + `law-updates.js` + `law-updates-data.js` | 계산 로직에 영향 주는 개정·시행령·정부 발표·판결을 일자별 타임라인으로 기록 — 영향 계산기 태그·엔진 반영 상태 표시, 자동 점검(daily-check·law-watch) 상태 배지 |
