...
node scripts/law-watch.js --days 62   # 동향 감시 기간 변경 (기본 31일 = 직전 한 달)
```

정기 감시는 `law-watch` 워크플로가 **매월 1일 09:00 KST**에 자동 실행합니다(월 1회로 API 비용 절감). 개정·시행예정 항목이나 신규 예규·판결이 발견되면 잡이 실패해 GitHub 알림이 가고, `law-watch` 라벨 이슈가 자동 생성되며, 리포트에 고쳐야 할 상수 위치(`constants.*`)가 표시됩니다. 리포트 끝의 「세법 동향 일지 등록 후보」에는 국세청 예규·질의응답, 기획재정부 유권해석, 판결·심판 결정이 `law-updates-data.js`에 바로 붙여 넣을 수 있는 JS 블록으로 정리되므로, 검토 후 `engineStatus`·`engineNote`만 채워 세법 동향 일지(`src/web/static/law-updates-data.js`)에 추가합니다.
...
## 온라인 세무 도구 (GitHub Pages)

정적 멀티페이지 사이트가 GitHub Pages에 자동 배포됩니다: **https://daehantax.github.io/property/**

세목·기능별 페이지로 구성된 도구 허브이며, 상단 내비게이션으로 이동합니다. 앞으로 판정기 등을 페이지로 계속 추가합니다.

모든 페이지의 상단 메뉴는 `nav.js` 하나가 그린다(세목별 2단 구성: **양도 · 상속 · 증여 · 취득 · 보유 · 상담 시나리오 · 세법 동향** → 그룹 안의 도구). 도구를 추가하면 `nav.js`의 `GROUPS`에 한 줄만 넣으면 메뉴와 `index.html` 하단 「세목별 도구 모음」이 함께 갱신된다. 메인 계산기 탭은 `index.html#gift` 처럼 해시로 바로 열 수 있다.
...
| **세법 동향** | `law-updates.html` + `law-updates.js` + `law-updates-data.js` | 계산 로직에 영향 주는 개정·시행령·정부 발표·판결·예규를 일자별 타임라인으로 기록 — 영향 계산기 태그·엔진 반영 상태 표시, 국세청 예규·질의응답·기획재정부 유권해석·조세심판원 공식 출처 바로가기, 자동 점검(daily-check·law-watch) 상태 배지 |
| **지방주택 1세대1주택 판정기** | `local-house.html` + `local-house.js` | 수도권 주택 1채 + 지방주택 1채 보유 세대의 소득세법·종부세법 1세대1주택 판정 (`local-house-judge.js`). 양도세: 일시적 2주택·상속·농어촌(§155①②⑦⑧), 조특법 농어촌·고향주택(§99의4, ~2028.12.31)·세컨드홈(§71의2, 인구감소지역 9억·관심지역 4억)·준공 후 미분양(§98의9) 7경로 + §154① + 중과 주택수 제외(3억) 표시. 종부세: §8④ 일시적 2주택·상속·지방 저가주택(4억·인구감소지역 9억) + 조특법 특례 → 12억 공제·1주택분 세액공제 적용 전후 세액 비교. 시·군·구 검색으로 인구감소지역 89곳·관심지역 18곳·접경지역 15곳·광역시 군 여부 자동 판별 (`regions-data.js`) |
...
- 로컬 서버(`npm run web`)는 같은 사이트를 서빙하며, 상담 시나리오 페이지는 **AI 정밀 보고서·심화 검토 버튼이 포함된 강화판**으로 제공됩니다(정적 사이트는 클라이언트 계산 전용).
