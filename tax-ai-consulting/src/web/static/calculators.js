...
const CALCULATORS = [
  ...
  },
  {
    ...
    intro: '1세대1주택 비과세(12억)·장기보유특별공제·조정지역 다주택 중과를 반영합니다. '
      + '양도(예정)연도를 선택하면 2027년 이후 양도분에는 2026 세제개편안(2026.8.3 정부안)의 개정 내용 — '
      + '다주택 중과 한시 완화(2027년 +5/+10%p, 2028년 +10/+15%p, 2029년 +20/+30%p 복귀), '
      + '장기거주소득공제 전환·공제한도(2028년~), 1세대1주택 기본공제 확대(2029년~) — 이 자동 반영됩니다. '
      + '※ 2027년 이후분은 국회 통과 전 정부안 기준',
    fields: [
      sel('saleYear', '양도(예정)연도', [
        { value: 2026, label: '2026년 — 현행법 (중과 +20/+30%p, 2026.5.10 부활)' },
        { value: 2027, label: '2027년 — 개정안 (중과 +5/+10%p 한시 완화)' },
        { value: 2028, label: '2028년 — 개정안 (중과 +10/+15%p · 장기거주소득공제 전환)' },
        { value: 2029, label: '2029년 이후 — 개정안 (중과 복귀 · 거주공제 연8% · 한도 10억)' },
      ], 2027),
    ...
    run: (v) => {
      // 2026년(현행)은 reformYear=0, 2027년부터는 개편안 레짐을 그대로 엔진에 전달
      const reformYear = v.saleYear >= 2027 ? v.saleYear : 0;
      const r = calcSaleIncomeTax(
        v.marketPrice, v.basePrice, v.holdPeriod, v.stayPeriod,
        v.isWvr, '주택', v.ownCount, v.isAdj, 0, '', 0, reformYear,
      );
      const REGIME_NOTE = {
        2026: '현행법 — 조정지역 다주택 중과 +20%p(2주택)/+30%p(3주택~), 장기보유특별공제(보유+거주 각 연4%)',
        2027: '개정안 — 중과 +5%p(2주택)/+10%p(3주택~) 한시 완화 · 장기보유특별공제는 현행 유지',
        2028: '개정안 — 중과 +10%p/+15%p · 장기거주소득공제 전환(1주택 보유 연2%+거주 연6%, 공제한도 20억)',
        2029: '개정안 — 중과 +20%p/+30%p 복귀 · 거주공제만 연8%(최대 80%, 한도 10억) · 10년 거주+30억 이하 기본공제 2,500만',
      };
      return {
        headline: r.total, headlineLabel: `양도세 + 지방소득세 합계 (${v.saleYear}년 양도 기준)`,
        sub: `양도소득세 ${won(r.transferTax)} + 지방소득세 ${won(r.localTax)}`
          + (r.breakdown.heavyApplied ? ' · 중과 적용' : ''),
        extraHtml: `<p style="font-size:12.5px;color:#566573;margin:6px 0 0">적용 기준: ${REGIME_NOTE[v.saleYear]}${
          reformYear ? '<br>※ 2027년 이후 양도분은 2026 세제개편안(정부안) 기준 — 국회 통과 시 조문 확정 후 재검토 필요' : ''}</p>`,
        computation: {
          kind: 'transfer',
          label: reformYear ? `양도소득세 (${v.saleYear}년 양도 — 개정안 반영)` : '양도소득세 (현행법)',
          result: r,
        },
      };
    },
  ...
  },
  {
    ...
    intro: '2026 세제개편안 확정안(2026.9.1 국무회의 의결·국회 제출안)에 따른 종합부동산세 변화를 '
      + '현행(2026) → 2027년(과도기) → 2028년~(단일세율) 순으로 비교합니다. '
      + '기본공제(실거주 14억 / 비거주 12억 유지 / 다주택 가액비례), 공동명의 인별 공제(거주 9억/비거주 6억), '
      + '공정시장가액비율(70~80%), 세율 일원화, 세액공제 한도(800만/600만), 세부담상한 150% 유지를 반영합니다. '
      + '확정안은 8.3 발표안에서 비거주 1주택 공제 인하(9억) 철회, 공동명의 비거주 인별 공제 4억→6억 조정, '
      + '세부담상한 인상 철회가 반영된 안입니다. ※ 국회 통과 전 확정안 기준',
    fields: [
      sel('oneOOne', '주택 유형', [
        { value: '1세대1주택', label: '1세대1주택' },
        { value: '공동명의1주택', label: '공동명의 1주택 (부부 인별 계산)' },
      ...
      sel('isResident', '실거주 여부 (1주택·공동명의)', [
        { value: 1, label: '실거주' },
      ...
      if (v.oneOOne === '공동명의1주택') {
        ...
        return {
          ...
          extraHtml: cmpTable(rows)
            ...
            + `(본인 ${pctA}% / 배우자 ${pctB}%)으로 계산해 합산했습니다. 확정안의 공동명의 인별 공제는 `
            + '거주 시 인별 9억(합 18억), 비거주 시 인별 6억(합 12억 — 당초 4억안에서 조정)이며 '
            + '위 「실거주 여부」 선택이 2027년분부터 반영됩니다.</p>',
          computations: [
